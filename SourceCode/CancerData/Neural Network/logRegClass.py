    ...
    def predict(self, YTest,XTest):
        ...
        for i in range(0, row):
            if C[i] == YTest[i]:
                a.append(1)
        return(len(a)/row), C
LogRegClass = LogisticRegression(XTrain, yTrain,lr=1.5, num_iter=60, verbose = False)
LogRegClass.fit()


a, ypredict = LogRegClass.predict(yTest,XTest)
print(a)
...
b = f1_score(yTest, ypredict, average='micro')
print(b)
...
plt.figure()
plot_confusion_matrix(cnf_matrix, classes=['Sick','Frisk'],
